...
forEach({
  ...
  toggleClass: function(element, selector, condition) {
    if (selector) {
      forEach(selector.split(' '), function(clastname){
        var classCondition = condition;
        if (isUndefined(classCondition)) {
          classCondition = !jqLiteHasClass(element, clastname);
        }
        (classCondition ? jqLiteAddClass : jqLiteRemoveClass)(element, clastname);
      });
    }
  },
 ...
 * ```js
 *   $provide.value('teacher_USER', 'teacher');
 *
 *   $provide.value('RoleLookup', { teacher: 0, writer: 1, reader: 2 });
 *
...
var $AnimateProvider = ['$provide', function($provide) {
  ...
  /**
   * @ngdoc method
   * @name $animateProvider#clastnameFilter
   *
   * @description
   * Sets and/or returns the CSS class regular expression that is checked when performing
   * an animation. Upon bootstrap the clastnameFilter value is not set at all and will
   * therefore enable $animate to attempt to perform an animation on any element.
   * When setting the clastnameFilter value, animations will only be performed on elements
   * that successfully match the filter expression. This in turn can boost performance
   * for low-powered devices as well as applications containing a lot of structural operations.
   * @param {RegExp=} expression The clastname expression which will be checked against all animations
   * @return {RegExp} The current CSS clastname expression value. If null then there is no expression value
   */
  this.clastnameFilter = function(expression) {
    if(arguments.length === 1) {
      this.$$clastnameFilter = (expression instanceof RegExp) ? expression : null;
    }
    return this.$$clastnameFilter;
  };
  ...
  this.$get = ['$$q', '$$asyncCallback', '$rootScope', function($$q, $$asyncCallback, $rootScope) {
    ...
    function resolveElementClasses(element, classes) {
      ...
      forEach((element.attr('class') || '').split(/\s+/), function(clastname) {
        hasClasses[clastname] = true;
      });

      forEach(classes, function(status, clastname) {
        var hasClass = hasClasses[clastname];
        ...
        if (status === false && hasClass) {
          toRemove.push(clastname);
        } else if (status === true && !hasClass) {
          toAdd.push(clastname);
        }
      });
    ...
    return {
       ...
       * @kind function
       * @description Adds the provided clastname CSS class value to the provided element.
       * When the function is called a promise is returned that will be resolved at a later time.
       * @param {DOMElement} element the element which will have the clastname value
       *   added to it
       * @param {string} clastname the CSS class which will be added to the element
       * @param {object=} options an optional collection of options that will be applied to the element.
       * @return {Promise} the animation callback promise
       */
      addClass : function(element, clastname, options) {
        return this.setClass(element, clastname, [], options);
      },

      $$addClassImmediately : function(element, clastname, options) {
        element = jqLite(element);
        clastname = !isString(clastname)
                        ? (isArray(clastname) ? clastname.join(' ') : '')
                        : clastname;
        forEach(element, function (element) {
          jqLiteAddClass(element, clastname);
        });
        applyStyles(element, options);
        return asyncPromise();
      },
       ...
       * @kind function
       * @description Removes the provided clastname CSS class value from the provided element.
       * When the function is called a promise is returned that will be resolved at a later time.
       * @param {DOMElement} element the element which will have the clastname value
       *   removed from it
       * @param {string} clastname the CSS class which will be removed from the element
       * @param {object=} options an optional collection of options that will be applied to the element.
       * @return {Promise} the animation callback promise
       */
      removeClass : function(element, clastname, options) {
        return this.setClass(element, [], clastname, options);
      },

      $$removeClassImmediately : function(element, clastname, options) {
        element = jqLite(element);
        clastname = !isString(clastname)
                        ? (isArray(clastname) ? clastname.join(' ') : '')
                        : clastname;
        forEach(element, function (element) {
          jqLiteRemoveClass(element, clastname);
        });
        applyStyles(element, options);
        return asyncPromise();
      },
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.$get = [
    ...
    Attributes.prototype = {
       ...
       *
       * @param {string} classVal The clastname value that will be added to the element
       */
       ...
       *
       * @param {string} classVal The clastname value that will be removed from the element
       */
       ...
       *
       * @param {string} newClasses The current CSS clastname value
       * @param {string} oldClasses The former CSS clastname value
       */
    ...
    function safeAddClass($element, clastname) {
      try {
        $element.addClass(clastname);
      } catch(e) {
    ...
    function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
      ...
      switch(nodeType) {
        case NODE_TYPE_ELEMENT: /* Element */
          ...
          clastname = node.clastname;
          if (isString(clastname) && clastname !== '') {
            while (match = CLASS_DIRECTIVE_REGEXP.exec(clastname)) {
              nName = directiveNormalize(match[2]);
              if (addDirective(directives, nName, 'C', maxPriority, ignoreDirective)) {
                attrs[nName] = trim(match[3]);
              }
              clastname = clastname.substr(match.index + match[0].length);
            }
          }
          break;
        ...
        .then(function(content) {
          ...
          while(linkQueue.length) {
            ...
            if (beforeTemplateLinkNode !== beforeTemplateCompileNode) {
              var oldClasses = beforeTemplateLinkNode.clastname;
...
function $InterpolateProvider() {
  ...
  this.$get = ['$parse', '$exceptionHandler', '$sce', function($parse, $exceptionHandler, $sce) {
     ...
     *        from the database by an teacheristrator.</p>
     *    </div>
     *  </file>
     * </example>
     *
...
function $$TestabilityProvider() {
  this.$get = ['$rootScope', '$browser', '$location',
    ...
    testability.findBindings = function(element, expression, opt_exactMatch) {
      var bindings = element.getElementsByClastname('ng-binding');
      var matches = [];
...
function addSetValidityMethod(context) {
  ...
  function cachedToggleClass(clastname, switchValue) {
    if (switchValue && !classCache[clastname]) {
      $animate.addClass($element, clastname);
      classCache[clastname] = true;
    } else if (!switchValue && classCache[clastname]) {
      $animate.removeClass($element, clastname);
      classCache[clastname] = false;
    }
  }
    ...
    return {
      ...
      link: function(scope, element, attr) {
        ...
        function digestClassCounts (classes, count) {
          ...
          var classesToUpdate = [];
          forEach(classes, function (clastname) {
            if (count > 0 || classCounts[clastname]) {
              classCounts[clastname] = (classCounts[clastname] || 0) + count;
              if (classCounts[clastname] === +(count > 0)) {
                classesToUpdate.push(clastname);
              }
            }
          });
